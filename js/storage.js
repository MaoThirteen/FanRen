...
function loadAll() {
  try {
    ...
    if (d.config.topP === undefined) d.config.topP = 0.9;
    ...
    if (!d.state.protagonist.bloodline) d.state.protagonist.bloodline = ['无'];
    if (!d.state.protagonist.daoRoot) d.state.protagonist.daoRoot = '金、木、水、火（伪灵根，主金系）';
    ...
    ['protagonist','companions','tempCharacters'].forEach(k => {
      ...
      arr.forEach(c => {
        ...
        if (!c.bloodline || c.bloodline === '无') c.bloodline = ['无'];
        if (!c.daoRoot) c.daoRoot = '';
        ...
        if (c.skills) for (let i = 0; i < c.skills.length; i++) { const a = c.skills[i]; if (typeof a === 'string') { const n = a.split('（')[0] || a; const ds = a.includes('（') ? a.match(/（([^）]*)）/)?.[1]||'' : '功能未知'; c.skills[i] = { name:n, grade:'凡间功法', desc:ds }; } }
      });
    });

    // 王铁迁移
    const wt = d.state.companions.find(c => c.name === '王铁');
    if (!wt) d.state.companions.unshift({ name:'王铁', realm:'炼气三层', gender:'男', bloodline:['金'], daoRoot:'天灵根，主金系', sect:'无', sectTitle:'散修', relation:'好友', exp:45, expMax:calcExpMax('炼气三层'), hp:85, hpMax:200, mp:35, mpMax:100, artifacts:[{ name:'开山斧', grade:'凡器', desc:'势大力沉' }], skills:[{ name:'铁布衫', grade:'凡间功法', desc:'淬炼肉身' }], spiritStones:20, inventory:[], status:'同行身旁' });
    else {
      ...
      if (!wt.skills) wt.skills = [{ name:'铁布衫', grade:'凡间功法', desc:'淬炼肉身' }];
      if (!wt.bloodline || wt.bloodline === '无') wt.bloodline = ['金'];
      if (!wt.daoRoot) wt.daoRoot = '天灵根，主金系';
...
data = loadAll();
if (!data) { data = { state:defaultState(), config:defaultConfig(), chatHistory:[], summaries:[], logs:[] }; saveAll(); }
